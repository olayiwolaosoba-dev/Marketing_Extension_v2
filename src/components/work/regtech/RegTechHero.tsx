...
import { ArrowRight, Shield, Activity, Users, Lock, Server } from 'lucide-react';
import { Link } from 'react-router-dom';

const RegTechHero: React.FC = () => {
    return (
            ...
            <div className="container mx-auto px-6 max-w-7xl relative z-10">
                <div className="max-w-4xl mx-auto text-center">
                    ...
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 text-primary text-xs font-bold uppercase tracking-widest mb-8">
                        <Server size={12} />
                        <span>RegTech & Cybersecurity Vertical</span>
                    </div>

                    <h1 className="text-5xl lg:text-7xl font-display font-bold text-text-dark mb-8 tracking-tight">
                        Growth engines for <br />
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-orange-600">security & compliance</span>
                    </h1>

                    <p className="text-xl lg:text-2xl text-text-dark/70 mb-12 max-w-3xl mx-auto leading-relaxed">
                        We help RegTech, identity, and cybersecurity platforms translate technical protection into business value for CISOs and boards.
                    </p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left max-w-4xl mx-auto border-t border-gray-200 pt-8">
                        <div className="flex flex-col gap-2">
                            ...
                            <span className="text-sm font-medium text-text-dark">CISOs · Risk Officers · DPOs · Legal</span>
                        </div>
                        <div className="flex flex-col gap-2">
                            <span className="text-xs font-bold uppercase tracking-widest text-text-muted flex items-center gap-2">
                                <Activity size={12} /> Sales Motion
                            </span>
                            <span className="text-sm font-medium text-text-dark">Heavy scrutiny · RFP-driven · Pilot-required</span>
                        </div>
                        <div className="flex flex-col gap-2">
                            <span className="text-xs font-bold uppercase tracking-widest text-text-muted flex items-center gap-2">
                                <Lock size={12} /> Trust Blockers
                            </span>
                            <span className="text-sm font-medium text-text-dark">Integration burden · Liability · Data residency</span>
                        </div>
                    </div>
